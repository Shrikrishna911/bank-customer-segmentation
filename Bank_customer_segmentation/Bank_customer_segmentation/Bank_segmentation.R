...
sum(is.na(data))
na.omit(data)
mean(data$CustAccountBalance, na.rm=TRUE)
...
barplot(gender_table, main="Gender Comparison",ylab = "Count",
        xlab = "CustGender", col = rainbow(3), legend = rownames(gender_table))
...
hist(customer_data$TransactionAmount..INR., main="HistoGram for Spending Score",
     xlab="STransaction amount Class", ylab="Frequency",
